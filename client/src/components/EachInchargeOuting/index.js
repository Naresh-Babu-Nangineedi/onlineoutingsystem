...
class EachInchargeOuting extends Component {
    ...
    rejectOuting=async(outing_id)=>{
       ...
       const inputData={rejectionReason}
        if(rejectionReason===""){
            ...
            const url = `/api/hostelIncharge/reject/${outing_id}`
            const options = {
                method:"POST",
            ...
            console.log(data)
            history.replace("/incharge/outings")
        }
    ...
    acceptOuting=async(outing_id)=>{
       const {history} = this.props
            ...
            const url = `/api/hostelIncharge/approve/${outing_id}`
            const options = {
                method:"POST",
                headers:{
                    "Authorization":Cookies.get("token")
    ...
    getOutingData=async(outing_id)=>{
        const url = `/api/hostelIncharge/outing/${outing_id}`
        const options={
            methos:"GET",
            headers:{
                "Authorization":Cookies.get("token")
...
export default withRouter(EachInchargeOuting)
